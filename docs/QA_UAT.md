...
durable Email/changes and Mailbox/changes; see [JMAP API](JMAP_API.md). Arbitrary-header
composition and thread collapsing/push remain open work. Delayed submission,
atomic cancellation and opt-in receipt/Sieve journal retention are implemented. Success email hooks,
query/receipt synchronization, anchored query pagination, bounded MIME trees/inline CID/part reuse and documented Sieve workflows are implemented. Acceptance receipts do not establish delivery or client UAT.
