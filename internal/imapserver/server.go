	...
	for update := range s.Updates {
		...
		for conn := range s.conns {
			...
			if *conn.silent() {
				...
				if _, ok := update.(*backend.MessageUpdate); ok {
					continue
				}
			}

			res := updateResponse(update)
			if res == nil {
				continue
			}
...
// updateResponse creates an independent response for each recipient. FETCH,
// EXPUNGE and LIST consume channels when written and cannot be shared.
func updateResponse(update backend.Update) imap.WriterTo {
