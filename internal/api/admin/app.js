...
async function request(path,method='GET',body){
 ...
 try{
  ...
  const text=await response.text();if(current!==generation)return;let display=text;try{display=JSON.stringify(JSON.parse(text),null,2);}catch{}
 ...
 }catch{if(current!==generation)return;el('status').textContent='Request failed. Check the connection and service status.';}
}
el('login').addEventListener('submit',e=>{e.preventDefault();generation++;credential=el('token').value;el('token').value='';request('queue/stats');});
el('logout').addEventListener('click',()=>{generation++;credential='';el('token').value='';el('password').value='';el('result').textContent='';el('status').textContent='Disconnected';});
