...
def run(binary, backup):
    with tempfile.TemporaryDirectory(prefix='mailhub-smoke-') as directory:
        ...
        print('PASS: SMTP/IMAP delivery, REST authorization/accounts/policies, folder mutations, JMAP upload/import, mailbox/keyword writes, email move/delete and change feeds, flags/search, shutdown and live restored-service verification')
...
if __name__=='__main__':
    ...
    if args.report:
        report = {
            'schema_version': 1,
            'scope': 'isolated live SMTP/IMAP, JMAP upload/import, mailbox/keyword writes, email move/delete and change feeds, REST and backup/restore qualification',
            'status': 'running',
